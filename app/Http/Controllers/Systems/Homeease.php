...
class Homeease extends Controller
{
    function createUser($user)
    {
        ...
        DB::connection('mysqlhease')->table('users')->insert($userArray);
    }
    function updateUser($user, $id)
    {
        DB::connection('mysqlhease')->table('users')->where('id', $id)->update($user);
    }
    public function forwardRequest(Request $request, $any)
    {
        $fullPath = $request->path();

        $targetUrl = rtrim(env('HOMEEASE_SERVICE')) . $fullPath;
        ...
        try {
            ...
            return response()->json(['message' => 'HomeEase Service unavailable'], 503);
        }
    }
}